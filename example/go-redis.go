...
func hashOperation(client *redis.Client) {
	client.HSet("user_xyz", "name", "xyz") // 向名称为 user_xyz 的 hash 中添加元素 name
	client.HSet("user_xyz", "age", "18")   // 向名称为 user_xyz 的 hash 中添加元素 age
	...
	fmt.Println("field count in user_xyz: ", length) // 字段个数为2
